      ...
      <div className="rounded-xl border bg-card overflow-x-auto">
        <table className="min-w-max w-full text-sm">
          <thead className="bg-secondary/50 text-xs uppercase tracking-wider text-muted-foreground">
            <tr>
              <th className="sticky left-0 z-10 bg-secondary/50 text-left pl-3 pr-2 py-2.5 font-semibold">#</th>
              <th className="sticky left-9 z-10 bg-secondary/50 text-left px-2 py-2.5 font-semibold">Squadra</th>
              <th className="text-right px-2 py-2.5 font-semibold">PT</th>
              <th className="text-right px-2 py-2.5 font-semibold">PG</th>
              <th className="text-right px-2 py-2.5 font-semibold">VD</th>
              <th className="text-right px-2 py-2.5 font-semibold">VS</th>
              <th className="text-right px-2 py-2.5 font-semibold">SS</th>
              <th className="text-right px-2 py-2.5 font-semibold">SD</th>
            ...
            {standings.map((s, i) => {
              ...
              return (
                <tr key={s.teamId} className="border-t hover:bg-secondary/30">
                  <td className="sticky left-0 z-10 bg-card pl-3 pr-2 py-2.5 hover:bg-secondary/30">
                  ...
                  <td className="sticky left-9 z-10 bg-card px-2 py-2.5 hover:bg-secondary/30">
                    <Link to="/squadre/$teamId" params={{ teamId: t.id }} className="flex items-center gap-2 hover:text-primary">
                      <TeamBadge teamId={t.id} size={26} />
                      <span className="font-semibold whitespace-nowrap">{t.name}</span>
                    </Link>
                  </td>
                  ...
                  <td className="px-2 py-2.5 text-right tabular-nums">{s.winsReg}</td>
                  <td className="px-2 py-2.5 text-right tabular-nums">{s.winsShoot}</td>
                  <td className="px-2 py-2.5 text-right tabular-nums">{s.lossesShoot}</td>
                  <td className="px-2 py-2.5 text-right tabular-nums">{s.lossesReg}</td>
